        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
            <div class="glass rounded-3xl p-6 border-dash-700">
                ...
                <div class="flex items-center space-x-3">
                    @php $mpKey = \App\Models\Setting::getValue('mp_access_token'); @endphp
                    @if($mpKey)
                        <span class="w-3 h-3 rounded-full bg-emerald-500 animate-pulse"></span>
                        <span class="text-sm font-bold text-emerald-400">Mercado Pago OK</span>
                    @else
            ...
            <div class="glass rounded-3xl p-6 border-dash-700">
                ...
                @php $enabled = \App\Models\Setting::getValue('mp_enabled', 'false') === 'true'; @endphp
                <div class="flex items-center space-x-3">
                    @if($enabled)
        ...
        <div class="glass rounded-[40px] border-dash-700 overflow-hidden shadow-2xl">
            <div class="px-10 py-8 border-b border-white/5">
                <div class="flex items-center space-x-4">
                    ...
                    <div class="flex-1">
                        <h4 class="text-lg font-bold text-white">Integração Mercado Pago</h4>
                        <p class="text-xs text-gray-500">Configure seu Access Token para receber pagamentos via PIX</p>
                    </div>
                    <form action="{{ route('admin.financeiro.test') }}" method="POST">
                        @csrf
                        <button type="submit" class="flex items-center space-x-2 bg-blue-600/10 border border-blue-500/20 text-blue-400 px-4 py-2 rounded-xl text-[10px] font-bold hover:bg-blue-600/20 transition">
            ...
            <form action="{{ route('admin.mercadopago.save') }}" method="POST" class="p-10 space-y-8">
                @csrf
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div>
                        <label class="block text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-3">Access Token do Mercado Pago</label>
                        <div class="relative" x-data="{ show: false }">
                            <input :type="show ? 'text' : 'password'" name="mp_access_token" 
                                value="{{ \App\Models\Setting::getValue('mp_access_token') }}"
                                class="w-full bg-[#0a0a0c] border border-white/5 rounded-2xl p-4 pr-12 text-white focus:ring-2 focus:ring-blue-500 outline-none transition-all font-mono text-sm"
                                placeholder="APP_USR-...">
                        ...
                        <p class="text-[10px] text-gray-600 mt-2">Encontre em: <span class="text-blue-400">Mercado Pago → Seu Negócio → Configurações → Gestão e Administração → Credenciais</span></p>
                    </div>
                    <div>
                        <label class="block text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-3">Opções Adicionais</label>
                        <div class="glass p-6 rounded-3xl border-dash-700 bg-emerald-500/5 h-[80px] flex items-center justify-between">
                            <div>
                            ...
                            <div class="relative inline-block w-12 h-6 transition duration-200 ease-in-out bg-gray-800 rounded-full cursor-pointer">
                                <input type="checkbox" name="mp_enabled" value="1" {{ $enabled ? 'checked' : '' }} class="opacity-0 w-0 h-0 peer absolute inset-0 cursor-pointer">
                                <span class="absolute top-1 left-1 bg-white w-4 h-4 rounded-full transition-all peer-checked:translate-x-6 peer-checked:bg-emerald-500 pointer-events-none"></span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 gap-8">
                    <div>
                        <label class="block text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-3">Webhook (Notificações)</label>
                        <div>
                            <p class="text-[10px] text-gray-500 mb-2">Sua Webhook URL (Cole nas configurações de Webhooks do Mercado Pago)</p>
                            <div class="bg-dash-900 border border-white/5 rounded-2xl p-4 flex items-center justify-between">
                                <code class="text-xs text-blue-400 font-mono">{{ url('/api/v1/webhook/mercadopago') }}</code>
                                <button type="button" @click="navigator.clipboard.writeText('{{ url('/api/v1/webhook/mercadopago') }}'); alert('URL Copiada!')" class="text-gray-500 hover:text-white">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012-2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z"></path></svg>
                                </button>
                            </div>
                            <p class="text-[10px] text-gray-600 mt-2">Certifique-se de assinalar o evento <strong>Pagamentos (Payments)</strong> na tela do Mercado Pago.</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="pt-4 border-t border-white/5 flex items-center justify-between">
                    ...
                    <button type="submit" class="bg-blue-600 hover:bg-blue-500 px-8 py-3 rounded-2xl font-bold text-sm text-white shadow-xl shadow-blue-900/20 transition">Salvar Configurações</button>
                </div>
            </form>
        </div>
