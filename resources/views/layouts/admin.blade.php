    ...
    <div x-data="{ open: false }" class="min-h-screen flex flex-col md:flex-row">
        ...
        <aside :class="open ? 'translate-x-0' : '-translate-x-full'" class="fixed md:relative md:translate-x-0 w-72 min-h-screen z-50 transition-transform duration-300 ease-in-out glass">
            <div class="p-8">
                ...
                <nav class="space-y-1">
                    ...
                    @if(auth()->user()->isAdmin())
                    <a href="{{ route('admin.financeiro') }}" class="flex items-center space-x-3 p-3 rounded-xl {{ request()->is('admin/financeiro*') ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-900/20' : 'hover:bg-dash-800 text-gray-400' }}">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z"></path></svg>
                        <span class="text-sm font-medium">Financeiro & Asaas</span>
                    </a>

                    <a href="{{ route('admin.users_progress') }}" class="flex items-center space-x-3 p-3 rounded-xl {{ request()->is('admin/usuarios/progresso*') ? 'bg-violet-600 text-white shadow-lg shadow-violet-900/20' : 'hover:bg-dash-800 text-gray-400' }}">
                        ...
                        <span class="text-sm font-medium">Uso dos Usuários</span>
                    </a>
                    
                    <a href="{{ route('admin.db_manager') }}" class="flex items-center space-x-3 p-3 rounded-xl {{ request()->is('admin/db-manager*') ? 'bg-red-600 text-white shadow-lg shadow-red-900/20' : 'hover:bg-dash-800 text-gray-400' }}">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 7v10c0 2.21 3.582 4 8 4s8-1.79 8-4V7M4 7c0 2.21 3.582 4 8 4s8-1.79 8-4M4 7c0-2.21 3.582-4 8-4s8 1.79 8 4m0 5c0 2.21-3.582 4-8 4s-8-1.79-8-4"></path></svg>
                        <span class="text-sm font-medium">Gerenciar Banco</span>
                    </a>
                    @endif
                    ...
                    <a href="{{ route('docs') }}" class="flex items-center space-x-3 p-3 rounded-xl {{ request()->is('documentacao*') ? 'bg-amber-600 text-white shadow-lg shadow-amber-900/20' : 'hover:bg-dash-800 text-gray-400' }}">
                        ...
                        <span class="text-sm font-medium">Documentação API</span>
                    </a>

                    <a href="{{ route('admin.tester') }}" class="flex items-center space-x-3 p-3 rounded-xl {{ request()->is('admin/tester*') ? 'bg-violet-600 text-white shadow-lg shadow-violet-900/20' : 'hover:bg-dash-800 text-gray-400' }}">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 20l4-4m0 0l4 4m-4-4V4"></path></svg>
                        <span class="text-sm font-medium">Simulador de API</span>
                    </a>
                </nav>
        ...
        <main class="flex-1 min-w-0 md:h-screen md:overflow-y-auto">
            ...
            <div class="p-8 md:p-12">
                ...
                @if(session('error'))
                    <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 5000)" class="bg-red-500/10 border border-red-500/20 text-red-400 p-4 rounded-2xl mb-8 flex items-center justify-between">
                        <div class="flex items-center space-x-3">
